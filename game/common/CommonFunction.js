...
const scaleGUI = (gui, obj, name) => {
    ...
    scaleFolder.add(obj.scale, 'x').min(0.1).max(2).step(0.1).name(`${name} S x`)
    scaleFolder.add(obj.scale, 'y').min(0.1).max(2).step(0.1).name(`${name} S y`)
    scaleFolder.add(obj.scale, 'z').min(0.1).max(2).step(0.1).name(`${name} S z`)
    return scaleFolder;
}
