...
import { UpdateMesh, addObjectToArray } from "./Different/UpdateMesh.js";
...
import { TransformControls } from '../cdn/jsm/TransformControls.js';
...
const init = () => {
  ...
  directionalLightM(gui, lightGroup, helperGroup, 37.1, 105.5, 0, 0.8, 0xffffff, lightGui);
  ...
  const glbLoader = (name) => {
    gltfLoader.load(
      ...
      (gltf) => {
        ...
        UpdateMesh(cityGroup, 'gui', mainModel);
      },
      (xhr) => {
        ...
        if (total === 100) {
          ...
          water = new Water(
            waterGeometry,
            {
              textureWidth: 512,
              textureHeight: 512,
              waterNormals: new THREE.TextureLoader().load('../static/texture/water_heightmap.jpg', function (texture) {
    ...
    if (playersPeer[socketName]) {
      ...
      if (sittingRayCaster.length > 0) {
        ...
        switch (sittingRayCaster[0].object.name) {
          /*-----------------1-------------------*/
          case 'demoMesh0': sittingM(playersPeerData, sittingObject[0].p, sittingObject[0].r); break;
          case 'demoMesh1': sittingM(playersPeerData, sittingObject[1].p, sittingObject[1].r); break;
          default:
            break;
        }
      }
